...
## 5.1 四種模式：Edit / Auto-Accept / Plan / Auto

Claude Code 有四種工作模式，用 **Shift+Tab** 循環切換。底部狀態列會顯示目前是哪個模式。
...
Auto Mode 是 2026 年 3 月新增的模式。Claude 自動決定要不要執行，背後有一個分類器監控每個動作。比 `--dangerously-skip-permissions` 更安全，因為它會擋住超出範圍的操作和 prompt injection。適合對自己的 CLAUDE.md 設定很有把握、不想每個動作都確認的資深用戶。

Shift+Tab 的循環順序：default → acceptEdits → plan → auto。
...
| Auto | 分類器把關，自動執行 | ⭐⭐⭐ | CLAUDE.md 設定完整的資深用戶 |

簡單來說：小改動用 Edit，大改動先 Plan 再 Edit，信任的重複作業才考慮 Auto-Accept。Auto Mode 是給已經把 CLAUDE.md 設定得很完整、信任自己的邊界設定的用戶。

我的使用比例大概是：Plan Mode 40%、Edit Mode 50%、Auto-Accept 10%。很多人一開始會覺得 Auto-Accept 很爽，但被 Claude 改壞兩次之後就學乖了。我通常只在改 test files 或者格式化的時候用 Auto-Accept。Auto Mode 我還在觀察，先讓 CLAUDE.md 設定夠穩再說。

---

## 5.2 Inline Diff：逐行 Review AI 的修改
...
## 5.3 Checkpoint & Rewind：不滿意就時間旅行
...
## 5.6 進階案例：用 Code 模式打造你的 Life OS

這個案例展示 Code 模式的威力不只在寫程式，而是**打造屬於你的自動化系統**。

Teresa Torres（產品管理領域的知名顧問）用 Claude Code 搭配 Obsidian，建了一套個人 Life OS。核心概念很簡單：每天早上打一個 `/today`，Claude 自動產出今天的待辦清單。
→ [Teresa Torres 的 Claude Code 系統](https://creatoreconomy.so/p/automate-your-life-with-claude-code-teresa-torres)

這個系統的背後，就是我們前面學的 Plan → Edit 工作流：

**Step 1：Plan Mode — 設計 `/today` 指令**

```
我想建一個 /today slash command。
每天早上執行時，它要：
1. 讀取 tasks/ 資料夾裡所有 .md 檔案
2. 找出 status 是 "in-progress" 或 "todo" 的任務
3. 按優先級排序
4. 產出一份今日待辦摘要，寫入 daily/YYYY-MM-DD.md
```

Claude 分析你的 Obsidian vault 結構，列出需要建的檔案和邏輯。

**Step 2：Edit Mode — 讓 Claude 建出指令**

Claude 建立 `.claude/commands/today.md`，裡面定義了完整的工作流。你逐個 review diff，確認邏輯沒問題。

**Step 3：試跑，迭代**

跑一次 `/today`，看看產出的摘要格式。不滿意？Rewind，調整 prompt，再跑一次。三次迭代之後，你就有一個完全符合你習慣的每日待辦系統。

這個例子有趣的地方是：你不需要學 Python、不需要學 JavaScript。你只需要用自然語言描述你要什麼，Claude 幫你把它變成可執行的指令。

### 更多 Code 使用場景

- **3 個 prompt 部署個人網站**：非工程師用自然語言描述，直接上線 → [Medium](https://mdy.medium.com/with-3-prompts-claude-helped-me-build-and-deploy-a-simple-web-app-no-coding-required-c1fae747db86)
- **15 分鐘做電影探索 App**：完整教學，可跟做 → [Creator Economy](https://creatoreconomy.so/p/claude-code-beginners-tutorial-build-a-movie-app-in-15-minutes)
- **個人記帳追蹤器**：從需求描述到可用的 web app → [Adventure PPC](https://www.adventureppc.com/blog/7-things-you-can-build-with-claude-code-in-under-an-hour-no-coding-experience-required)
- **RPG 學習平台**：一個人一個 session 做出 5 世界 20 關卡的原型 → [SaaS City](https://saascity.io/blog/10-wildest-claude-code-projects-going-viral)
- **AI 照片搜尋**：用自然語言找照片（「找日落那張」）→ [Code Like a Girl](https://codelikeagirl.substack.com/p/claude-code-project-ideas-beginner-to-advanced)
...
第六章我們聊 CLAUDE.md——如何把你的規範寫進 Claude 的記憶，讓每次對話都從正確的起點開始。
