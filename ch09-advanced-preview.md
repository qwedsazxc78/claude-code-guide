# 第九章：進階 — MCP、Commands、Agents、Skills

*從使用工具，到設計系統*

前八章教你跟 Claude Code 工作：對話、Cowork 處理檔案、改 code、配置環境、背景執行、日常工作流。這些是基礎。
...
> **本章涵蓋的 4 個擴充功能（第五章教了前三個，這裡教後四個）：**
>
> | 功能 | 一句話 | 比喻 | 本章小節 |
> |------|--------|------|---------|
> | **MCP** | 讓 Claude 連接外部服務 | 辦公室的電話分機 | 8.1 |
> | **Commands** | 把重複的 prompt 存成指令 | SOP 文件 | 8.2 |
> | **Agents** | 讓 Claude 扮演特定角色 | 部門專員 | 8.3 |
> | **Skills** | 自動載入領域知識 | 培訓教材 | 8.4 |
>
> 不確定該用哪個？看 [附錄 A：初學者決策指南](appendix-a-decision-guide.md)。

---

## 9.1 MCP — 讓 Claude 連接外部世界
...
你會發現一個趨勢：越來越多人選擇用 CLI 工具直接透過 Bash 呼叫，而不是走 MCP Server。

為什麼？因為大部分外部服務都有自己的 CLI 工具，而 Claude Code 本來就能跑 Bash。

```bash
# 用 GitHub CLI，不需要 MCP
gh issue list --repo my-org/my-repo --state open
gh pr create --title "fix: 修正 login 流程" --body "..."

# 用 Linear CLI
linear issue list --team DEV --state started

# 用 Slack CLI
slack chat send --channel "#deploy" --text "部署完成"
```

MCP 多了一層抽象：你需要啟動一個 Server 程序、管理連線、處理 token，而且每個 MCP Server 開著就佔 context window。CLI 工具直接呼叫、用完即走，沒有這些額外成本。
...
| 該服務有成熟的 CLI | ✅ | |
| 需要持續監聽或串流資料 | | ✅ |
| 操作是一次性的 query/command | ✅ | |
| 需要跨多個 API 做複雜串接 | | ✅ |
| 團隊已經熟悉該 CLI | ✅ | |
| 服務沒有 CLI，只有 API | | ✅ |

簡單來說：**能用 CLI 就用 CLI，CLI 做不到的再開 MCP。** 這跟你選工具的邏輯一樣——先用簡單的，不夠用了再升級。

實務上，我自己現在的做法是：GitHub 用 `gh` CLI、Linear 用 `linear` CLI、Docker 用 `docker` CLI。MCP 只留給那些真的沒有 CLI、或者需要持續連線的服務。

---

## 9.2 Commands — 可重用的工作流（完整拆解）
...
### 趨勢觀察：Skill 作為 SOP 執行器

最早 Skill 的定位是「背景知識」——讓 Claude 記住你的規範和偏好。但現在的用法正在演化：**Skill 越來越像是可自動觸發的 SOP（Standard Operating Procedure）。**

跟 Command 比，Skill 的觸發方式更自然。你不需要記住指令名稱、手動打 `/xxx`，只要在對話中提到相關的情境，Claude 就會自動載入對應的 Skill 並照著流程走。

舉個例子：我有一個 GitBook 寫作的 Skill，裡面定義了 GitBook 的區塊語法規則、編輯守則、內容建立流程。當我說「幫我改這個頁面的 FAQ 區塊」，Claude 自動載入這個 Skill，知道要用 `<details>/<summary>` 格式、知道要保留 GitBook 特有語法、知道段落不能超過三行。

這其實就是 SOP 的概念：**標準化流程 + 自動觸發 + 品質檢查。**

**Skill 跟 Command 的分工正在重新定義：**

| | Command | Skill（SOP 模式） |
|--|---------|------------------|
| 觸發方式 | 手動 `/指令` | 自動偵測情境 |
| 適合場景 | 明確的一次性動作 | 持續性的工作流程規範 |
| 更新頻率 | 寫好就不太改 | 隨著踩坑持續迭代 |
| 典型用途 | 初始化、生成、部署 | 寫作規範、review 標準、格式檢查 |

如果你發現自己在每個 Command 裡都重複寫同一段「品質檢查」邏輯，那段邏輯應該抽出來變成 Skill。讓 Skill 當 SOP 執行器，Command 只負責觸發動作。
