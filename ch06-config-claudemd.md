# 第六章：Config — 把 Claude 配置成你的專屬工程師

*CLAUDE.md 是你跟 AI 的合約*

前五章你學會了怎麼跟 Claude 聊天、用 Cowork 處理檔案、讓它動手改 code。
...
> **快速導覽：Claude Code 的 6 個擴充功能**
>
> | 功能 | 一句話 | 比喻 |
> |------|--------|------|
> | **CLAUDE.md** | 讓 Claude 記住專案規則 | 新人入職手冊 |
> | **Global CLAUDE.md** | 讓 Claude 記住你的個人偏好 | 你的個人簡歷 |
> | **Hooks** | Claude 做某件事時自動觸發 | 門禁系統 |
> | **Commands** | 把重複的 prompt 存成指令 | SOP 文件 |
> | **Agents** | 讓 Claude 扮演特定角色 | 部門專員 |
> | **Skills** | 自動載入領域知識 | 培訓教材 |
>
> 這一章教前三個（CLAUDE.md、Hooks、Settings）。Commands、Agents、Skills 在第九章。
> 完整的決策指南在 [附錄 A](appendix-a-decision-guide.md)。

---

## 6.1 CLAUDE.md — AI Coding 的靈魂
...
```json
{
  "hooks": {
    "PreToolUse": [
      {
        "matcher": "Bash",
        "command": "echo \"$COMMAND\" | grep -qE 'rm\\s+-rf' && exit 2 || exit 0"
      }
    ]
  }
}
...
只要 Claude 想跑任何包含 `rm -rf` 的指令，直接擋住。exit code `2` = 阻止，exit code `0` = 繼續。exit code `1` 是「hook 本身執行失敗」的非阻斷性錯誤，Claude 的動作還是會繼續進行。
