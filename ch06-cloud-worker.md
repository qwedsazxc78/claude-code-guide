# 第六章：Cloud Worker — 背景執行與平行開發

SRE 出身的人有一個根深蒂固的直覺：卡住等待是在浪費資源。

你跑一個 CI pipeline，不會傻傻盯著螢幕等它跑完。你推一個 build，不會停下來等 deploy 結束。你知道平行處理是常識，不是優化。

Claude Code 的 Cloud Worker 功能，就是把這個邏輯帶進 AI 開發工作流。這一章我們聊四件事：Background Agent 讓你的 AI 變成平行 process、Headless Mode 讓 Claude 進入 CI/CD pipeline、Agent Teams 的現實評估、還有 CoWork——讓不寫 code 的人也能用 Claude Code 的力量。

---

## 6.1 Background Agent — AI 的 parallel process
...
## 6.2 Headless Mode — 讓 AI 進入 CI/CD Pipeline
...
我自己在 Gaia 內容系統建了三套 multi-agent 系統，這裡先留個預告：

- **Hera**：7 個 agents，負責網站設計與前端開發（ai-web-template）
- **Athena**：9 個 agents，負責全端 SaaS 開發、測試、部署（ai-coding-template）
- **Gaia**：4 個 agents + 69 個 commands，負責內容飛輪的整個生命週期

這三套系統的核心不是 Agent Teams 這個實驗功能，而是 Subagents + Background Agent + 明確的 CLAUDE.md 邊界定義。進階內容我們之後再聊。

---

## 6.4 CoWork — 不寫 code 也能用
...
|------|------------|--------|
| 目標用戶 | 工程師 | 所有人 |
| 介面 | VS Code / Terminal | Claude Desktop App |
| 執行環境 | 完整開發環境 | 安全沙盒 |
| 能跑指令 | 所有指令 | 受限制的安全指令 |
| 能安裝套件 | 可以 | 有限 |
| 方案要求 | Pro 以上 | Max（推薦）|
...
```
你：「把 Downloads 裡的 100 個檔案整理好，
     PDF 放 documents/，圖片放 images/，
     程式碼放 code/」
Claude：（自動分析、分類、搬移）
```
...
```
你：「把這 20 個 CSV 合併成一份 Excel，
     加上 pivot table，按地區分組統計營收」
Claude：（寫 Python script、執行、產出 Excel）
```

**整理會議紀錄**

```
你：「讀取 meeting-notes/ 裡這週的所有會議紀錄，
     整理成 weekly summary，
     列出所有 action items 和負責人」
Claude：（讀所有紀錄、分析、產出整理報告）
```
...
你：「把這資料夾裡的 200 張照片，
     根據 EXIF 拍攝日期重新命名，
     格式是 YYYY-MM-DD_HH-MM-SS.jpg」
Claude：（讀取 EXIF、批次重命名）
```

### Alex 用 CoWork 示範給 USL CEO 的那次
...
CoWork 不是工程師的玩具，是讓 PM、設計師、業務、財務也能用 Claude Code 能力的入口。如果你有不寫 code 的同事，花 10 分鐘帶他們看一次 CoWork，他們會感謝你。
...
**重點來了**：Cloud Worker 的本質是「用平行化換時間」。從 SRE 的角度看，這跟你管 production 系統時跑多個 monitoring job 的思路完全一樣——每個 job 各司其職，互不干擾。掌握了這個能力，你的開發速度不是線性成長，而是倍數成長。
