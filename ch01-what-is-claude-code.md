...
**AI Coding 的時代來了，但大多數人還在用錯誤的方式用 AI。**
...
## 1.4 三種介面：Chat / Cowork / Code

Claude Desktop App 上方有三個 tab，對應三種使用方式：

| Tab | 適合誰 | 做什麼 | 本書對應 |
|-----|--------|--------|---------|
| **Chat** | 所有人 | 對話、問答、分析 | 第三章 |
| **Cowork** | 所有人（含非工程師） | 處理本地檔案、整理資料 | 第四章 |
| **Code** | 工程師 | 寫程式、改 code、跑測試 | 第五章 |

除了 Desktop App，Claude Code 還有兩種開發者介面：

| 介面 | 適合誰 | 功能完整度 |
|------|--------|-----------|
| **VS Code Extension** | 所有開發者 | 90% |
| **Terminal CLI** | 進階用戶 | 100% |

**VS Code Extension** 是我推薦的工程師入門方式。不用離開熟悉的 IDE，所有功能都在側邊欄，視覺化操作。本書大部分 Code 相關內容都以這個介面示範。

**Terminal CLI** 是功能最完整的方式。支援 Headless 模式（無介面執行）、在 CI/CD pipeline 裡跑、批次處理任務。

**Cowork** 是給不寫 code 的人的入口。第四章會完整介紹，你會看到：不需要任何技術背景，只要會打中文，就能讓 AI 幫你整理 2000 個檔案。

我個人 80% 用 VS Code Extension，15% 用 Terminal（跑 Background Agent 和 CI/CD），5% 用 Cowork（示範給非工程師客戶看的時候）。
...
| **第四章** Cowork — 不寫 code 也能用 AI | 處理本地檔案、整理 Downloads、收據自動記帳，非工程師也能上手 |
| **第五章** Code — 讓 Claude 動手改你的 Code | 用 Plan Mode、Inline Diff、Checkpoint 安全地讓 AI 改你的 code |
| **第六章** Config — 把 Claude 配置成你的專屬工程師 | 寫 CLAUDE.md 和 Hooks，讓 Claude 理解你的規範、自動化重複流程 |
| **第七章** 背景執行與平行開發 | 開 Background Agent 處理大任務，用多 Agent 同時推進不同工作 |
| **第八章** 我的日常工作流與踩坑指南 | 看到一個真實工程師如何把 Claude Code 整合進每日開發節奏 |
| **第九章** 進階：MCP、Agents、Skills | 連接外部工具、建立自訂 Agent、打造屬於你的 AI 工作流擴充 |
